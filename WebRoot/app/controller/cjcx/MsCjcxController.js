Ext.define('App.controller.cjcx.MsCjcxController', {
    extend: 'Ext.app.Controller',
    
    models: ['cjcx.ViewJxCjcxModel'],
    stores: ['cjcx.MsCjcxStore'],
...
//    
    init: function() {
		this.control({
			'msCjcxList gridtoolbar combo[itemId=numCmb]': {
					render: this.initPageSize,
					select: this.setPageSize
			},
			
			'msCjcxList button[itemId=schShowBtn]': {
				render: this.setSchShowText
			},
			
			'msCjcxList button[action=showSearch]': {
				click: this.showSearch
			},
			
			'msCjcxList button[action=display]':{
				click:this.display
			},
			
			'msCjcxList button[action=toExcel]': {
...
//    	contentPanel.down('#xq').setValue(xq);
    ...
    onLaunch:function(record){
		...
		var contentPanel = Ext.create('App.view.cjcx.MsCjcxConPan',
				{
					itemId : 'content',
    ...
    search: function( o, e, eOpts){
		...
		var pGrid = tab.down('#msCjcxList'), store = me.getStore('msCjcxStore');
...
//		
		...
		store.load();store.load({callback:function(){
					var toolbar = pGrid.down('#toolbar');
					toolbar.moveFirst()
		}});
    },
    ...
    searchAll: function( o, e, eOpts){
    	...
    	var tab = me.getCenter().getActiveTab();
		var searchForm = tab.down('#searchForm');
    	var pGrid = tab.down('#msCjcxList');
    	searchForm.getForm().reset();
    	
    	var store = me.getStore('msCjcxStore'), proxy = store.getProxy();
