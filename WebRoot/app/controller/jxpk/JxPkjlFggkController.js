Ext.define('App.controller.jxpk.JxPkjlFggkController',{
	extend:'Ext.app.Controller',
	
	models:['jxpk.JxPkjlModel','skxx.SKXXModel','jxpk.PksjModel'],	
    stores: ['jxpk.JxPkjlStore','jxpk.SkxxStore','jxpk.JxPkjlCtStore','jxpk.PksjStore','jxpk.JxPkjlAllStore'],
	
	init:function(){
		this.control({
			'#skxxFggkList button[itemId=schShowBtn]' : {
				render : this.setSchShowText
			  },
		   '#skxxFggkList button[action=showSearch]' : {
				      click : this.showSearch
			},
			'#pkFggkSearchForm button[action=search]':{
				click: this.search
			},
			'#pkFggkSearchForm button[action=searchAll]':{
				click: this.searchAll
			},
			'#skxxFggkList button[action=skxxDetail]':{
				click:this.skxxDetail
			},
			'#skxxFggkList button[action=arrangeCourseList]':{
				click:this.arrangeCourseList
			},
			'#skxxFggkList button[action=arrangeCourseGrid]':{
				click:this.arrangeCourseGrid
			},
			'#jxPkjlFggkList button[action=delete]':{
				click:this.deletePkjl
			},
			'#jxPkjlFggkList button[action=pkjlEdit]':{
					click:this.editPkjl
			},
			'#jxPkjlFggkList button[action=pkjlDetail]':{
				click:this.detailPkjl
			},
			'#jxPkjlFggkList button[action=exportCourse]':{
				click:this.exportCourse
			}
		});
	},
			exportCourse : function(o, e, eOpts) {
				var list = o.up('#jxPkjlFggkList');
				var store = list.getStore();
				var JSONList = [];
				store.each(function(rec){
						JSONList.push(JSON.stringify(rec.data));
				})
				var downloader = list.down('#fileDownloader');
        		downloader.load({
        			params: {datas:JSONList},
        			url: 'jxPkjlExportCourse.action'
        		});
			},
			...
			search : function(o, e, eOpts) {
				...
				Ext.Ajax.request({
					...
					var searchForm = tab.down('#pkFggkSearchForm');
					var searchParams = {
						start : 0,
						limit : 15,
						page : 1,
					...
					var skxxFggkList = tab.down('#skxxFggkList'), skxxStore = skxxFggkList.getStore();
					var pkjlFggkList = tab.down('#jxPkjlFggkList'), pkjlStore = pkjlFggkList.getStore();
				...
				if (recs.length == 0) {
					...
					if(pkztm == '1'){
						...
						if(roleCode == 'Academy' && ggkbzm == '1'){
							...
							var skxxFggkList = o.ownerCt.ownerCt.ownerCt.ownerCt.down('#skxxFggkList');
							var pkjlFggkList = o.ownerCt.ownerCt.ownerCt.ownerCt.down('#jxPkjlFggkList');
							var application = me.getApplication();
			        		var controller = application.getController('App.controller.jxpk.PKEditController');
					  	    controller.onLaunch(o,skxxFggkList,pkjlFggkList);
						}
					}
				}
...
//				pkjlAllStore.load();
			...
			onLaunch : function(record) {
				...
				var contentPanel = Ext.create('App.view.jxpk.PKFggkContentPanel',{
							itemId : 'content',
					...
					url:'jxPksjCheckSj.action',
 		 			...
 		 			success : function(response, opts) {
						...
						if(success){
							...
							var pkjlFggkList  = contentPanel.down('#jxPkjlFggkList');
							var skxxFggkList = contentPanel.down('#skxxFggkList');
							
							var pkjlStore  = pkjlFggkList.getStore(),pkjlProxy = pkjlStore.getProxy();
							var skxxStore  = skxxFggkList.getStore(),skxxProxy = skxxStore.getProxy();
							
							pkjlProxy.setExtraParam('pkType', 'fggk');
							skxxProxy.setExtraParam('pkType', 'fggk');
							
							skxxFggkList.pkjlList = pkjlFggkList
							
							me.initStore();
						}else{
							Ext.MessageBox.alert('提示', '当前不在排课时间范围内！');
							contentPanel.down('#skxxFggkList').setDisabled(true);
							contentPanel.down('#jxPkjlFggkList').setDisabled(true);
						}
					}
				})
