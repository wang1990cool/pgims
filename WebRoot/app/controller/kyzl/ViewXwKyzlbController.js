Ext.define('App.controller.kyzl.ViewXwKyzlbController', {
	extend : 'Ext.app.Controller',

	models : ['kyzl.ViewXwKyzlbModel', 'xs.XsJcxxbModel','kyzl.XwKyzlbModel','kyzl.XwKyzlzlbModel'],
	stores : ['zd.ZdFwqbStore','kyzl.ViewXwKyzlbStore','zd.ZdSztmStore','xs.XsJcxxbStore','kyzl.XwKyzlbStore','kyzl.XwKyzlzlbStore'],

	refs : [{
				selector : 'viewXwKyzlbSearchForm',
				ref : 'viewXwKyzlbSearchForm'
			}],
	...
	init : function() {
		this.control({
					...
					'#viewXwKyzlbList gridtoolbar combo[itemId=numCmb]' : {
						render : this.initPageSize,
						select : this.setPageSize
					},
					'#viewXwKyzlbList gridtoolbar button[action=detail]' : {
						click : this.detail
					},
					'#viewXwKyzlbList  button[action=add]' : {
						click : this.Add
					},
					'#viewXwKyzlbList  button[action=tjedit]' : {
						click : this.tjedit
					},
					'#viewXwKyzlbList  button[action=tjchedit]' : {
						click : this.tjchedit
					},
					'#xwLwkyzlzlbList  button[action=zlUpload]' : {
						click : this.zlUpload
					},
					'#viewXwKyzlbList button[action=delete]' : {
						click : this.del
					},
					'#viewXwKyzlbList gridtoolbar button[action=editsj]' : {
						click : this.editClass
					},
					'#viewXwKyzlbList gridtoolbar button[action=delete]' : {
						click : this.deletexx
					},
					 '#viewXwKyzlbList gridtoolbar button[action=searchXh]': {
					 click : this.searchXh
					 },
					'#viewXwKyzlbList gridtoolbar button[itemId=schShowBtn]' : {
						render : this.setSchShowText
					},

					'#viewXwKyzlbList gridtoolbar button[action=showSearch]' : {
	...
	initStore : function() {
		var me = this;
		var store = me.getStore('ViewXwKyzlbStore', {});
		var searchParams = {
			start : 0,
			limit : 15,
			page : 1,
	...
	excelBtnRender : function(o, e, eOpts) {
		o.actionUrl = 'viewXwKyzlbtoExcel.action'
	},
	...
	onLaunch : function(record) {
		...
		var contentPanel = Ext.create(
				'App.view.kyzl.ViewXwKyzlbContentPanel', {
					itemId : 'content',
	...
	/*detail : function(o, e, eOpts) {
		...
		if(rec == undefined){Ext.MessageBox.alert('提示', '请选择一条数据记录！');}
			...
			var win = new Ext.Window({
						...
						animCollapse : true,
						autoShow : true,
						items : [Ext.create('App.view.kyzl.ViewXwKyzlbForm')]
					});
			var viewXwKyzlbshForm = win.down('#viewXwKyzlbForm');
			viewXwKyzlbshForm.loadForm(rec)
			 var sj = rec.data.lrsj;
			 var yearsj = sj.substring(0,10);
			 var monthsj = sj.substring(11,19);
			 lrsj = yearsj+ " " +monthsj;
			viewXwKyzlbshForm.down('#lrsj').setValue(lrsj);
			
		}
		var textfields = viewXwKyzlbshForm.query('textfield');
		...
		var me = this;
		var xh = rec.data.xh
		var xwKtbgzlbStore = me.getStore('XwLwkyzlzlbStore');
	...
	Add:function(o, e, eOpts){
					var me = this;
					var skxxList = o.up('#viewXwKyzlbList')
					var application = me.getApplication();
	        		var controller = application.getController('App.controller.kyzl.ArrangeTecController');
	...
	editClass : function(o, e, eOpts){
					...
					var skxxList = o.up('#viewXwKyzlbList')
					var application = me.getApplication();
	        		var controller = application.getController('App.controller.kyzl.ArrangeTecZlController');
...
detail : function(o, e, eOpts){
	...
	tjedit : function(o, e, eOpts) {
		...
		Ext.MessageBox.alert('提示', '请选择一条数据记录！');
			...
			var shjg = "";
			Ext.Ajax.request({
						url : 'xwKyzlbchangeFlowFlag.action',
						method : 'get',
						params : {
							xh : xh,
							kyuuid : kyuuid,
						...
						success : function(response) {
							...
							var viewXwKyzlbLists = Ext.ComponentQuery
									.query('#viewXwKyzlbList');
							if (viewXwKyzlbLists.length > 0) {
								var viewXwKyzlbList = viewXwKyzlbLists[0];
								var store = viewXwKyzlbList.getStore();
								store.load();
							}
						},
...
//			},
	...
	search : function(o, e, eOpts) {
		...
		var pGrid = tab.down('#viewXwKyzlbList'), store = me
				.getStore('ViewXwKyzlbStore');
...
//				});
	...
	searchAll : function(o, e, eOpts) {
		...
		var searchForm = tab.down('#searchForm');
		var pGrid = tab.down('#viewXwKyzlbList');
