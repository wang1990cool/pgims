Ext.define('App.controller.kyzl.ViewXwKyzlbzsController', {
	...
	refs : [{
				selector : 'viewXwKyzlbzsSearchForm',
				ref : 'viewXwKyzlbzsSearchForm'
			}],
	...
	init : function() {
		this.control({
					...
					'#viewXwKyzlbzsList gridtoolbar combo[itemId=numCmb]' : {
						render : this.initPageSize,
						select : this.setPageSize
					},
					'#viewXwKyzlbzsList gridtoolbar button[action=detail]' : {
						click : this.detail
					},
					'#viewXwKyzlbzsList  button[action=tjedit]' : {
						click : this.tjedit
					},
					'#viewXwKyzlbzsList gridtoolbar button[action=editsj]' : {
						click : this.editClass
					},
					'#viewXwKyzlbzsList gridtoolbar button[itemId=schShowBtn]' : {
						render : this.setSchShowText
					},

					'#viewXwKyzlbzsList gridtoolbar button[action=showSearch]' : {
	...
	onLaunch : function(record) {
		...
		var contentPanel = Ext.create(
				'App.view.kyzl.ViewXwKyzlbzsContentPanel', {
					itemId : 'content',
	...
	editClass : function(o, e, eOpts){
					...
					var skxxList = o.up('#viewXwKyzlbzsList')
					var application = me.getApplication();
	        		var controller = application.getController('App.controller.kyzl.ArrangeTeczlzsController');
...
detail : function(o, e, eOpts){
					...
					var skxxList = o.up('#viewXwKyzlbzsList')
					var application = me.getApplication();
	        		var controller = application.getController('App.controller.kyzl.ArrangeTeczlxqzsController');
			  	    controller.onLaunch(o,skxxList,kyuuid);
					}
			},
	...
	tjedit : function(o, e, eOpts) {
		...
		Ext.MessageBox.alert('提示', '请选择一条数据记录！');
			...
			var shjg = "";
			Ext.Ajax.request({
						url : 'xwKyzlbchangeFlowFlagg.action',
						method : 'get',
						params : {
							xh : xh,
							kyuuid : kyuuid,
						...
						success : function(response) {
							...
							var viewXwKyzlbzsLists = Ext.ComponentQuery
									.query('#viewXwKyzlbzsList');
							if (viewXwKyzlbzsLists.length > 0) {
								var viewXwKyzlbzsList = viewXwKyzlbzsLists[0];
								var store = viewXwKyzlbzsList.getStore();
								store.load();
							}
						},
	...
	search : function(o, e, eOpts) {
		...
		var pGrid = tab.down('#viewXwKyzlbzsList'), store = me
...
//				});
	...
	searchAll : function(o, e, eOpts) {
		...
		var searchForm = tab.down('#searchForm');
		var pGrid = tab.down('#viewXwKyzlbzsList');
