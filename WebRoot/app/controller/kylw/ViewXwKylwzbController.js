Ext.define('App.controller.kylw.ViewXwKylwzbController', {
	extend : 'Ext.app.Controller',

	models : ['kylw.ViewXwKylwzbModel', 'xs.XsJcxxbModel','kylw.XwKylwbModel','kylw.XwKylwzlbModel'],
	stores : ['zd.ZdFwqbStore','kylw.ViewXwKylwzbStore','zd.ZdSztmStore','xs.XsJcxxbStore','kylw.XwKylwbStore','kylw.XwKylwzlbStore'],

	refs : [{
				selector : 'viewXwKylwzbSearchForm',
				ref : 'viewXwKylwzbSearchForm'
			}],
	...
	init : function() {
		this.control({
					...
					'#viewXwKylwzbList gridtoolbar combo[itemId=numCmb]' : {
						render : this.initPageSize,
						select : this.setPageSize
					},
					'#viewXwKylwzbList gridtoolbar button[action=detail]' : {
						click : this.detail
					},
					'#viewXwKylwzbList  button[action=add]' : {
						click : this.Add
					},
					'#viewXwKylwzbList  button[action=tjedit]' : {
						click : this.tjedit
					},
					'#viewXwKylwzbList  button[action=tjchedit]' : {
						click : this.tjchedit
					},
					'#xwLwkylwzlbList  button[action=zlUpload]' : {
						click : this.zlUpload
					},
					'#viewXwKylwzbList button[action=delete]' : {
						click : this.del
					},
					'#viewXwKylwzbList gridtoolbar button[action=editsj]' : {
						click : this.editClass
					},
					'#viewXwKylwzbList gridtoolbar button[action=delete]' : {
						click : this.deletexx
					},
					 '#viewXwKylwzbList gridtoolbar button[action=searchXh]': {
					 click : this.searchXh
					 },
					'#viewXwKylwzbList gridtoolbar button[itemId=schShowBtn]' : {
						render : this.setSchShowText
					},

					'#viewXwKylwzbList gridtoolbar button[action=showSearch]' : {
	...
	initStore : function() {
		var me = this;
		var store = me.getStore('ViewXwKylwzbStore', {});
		var searchParams = {
			start : 0,
			limit : 15,
			page : 1,
	...
	excelBtnRender : function(o, e, eOpts) {
		o.actionUrl = 'viewXwKylwzbtoExcel.action'
	},
	...
	onLaunch : function(record) {
		...
		var contentPanel = Ext.create(
				'App.view.kylw.ViewXwKylwzbContentPanel', {
					itemId : 'content',
	...
	/*detail : function(o, e, eOpts) {
		...
		if(rec == undefined){Ext.MessageBox.alert('提示', '请选择一条数据记录！');}
			...
			var win = new Ext.Window({
						...
						animCollapse : true,
						autoShow : true,
						items : [Ext.create('App.view.kylw.ViewXwKylwzbForm')]
					});
			var viewXwKylwzbshForm = win.down('#viewXwKylwzbForm');
			viewXwKylwzbshForm.loadForm(rec)
			 var sj = rec.data.lrsj;
			 var yearsj = sj.substring(0,10);
			 var monthsj = sj.substring(11,19);
			 lrsj = yearsj+ " " +monthsj;
			viewXwKylwzbshForm.down('#lrsj').setValue(lrsj);
			
		}
		var textfields = viewXwKylwzbshForm.query('textfield');
		...
		var me = this;
		var xh = rec.data.xh
		var xwKtbgzlbStore = me.getStore('XwLwkylwzlbStore');
	...
	Add:function(o, e, eOpts){
					var me = this;
					var skxxList = o.up('#viewXwKylwzbList')
					var application = me.getApplication();
	        		var controller = application.getController('App.controller.kylw.ArrangeTecController');
	...
	editClass : function(o, e, eOpts){
					...
					var skxxList = o.up('#viewXwKylwzbList')
					var application = me.getApplication();
	        		var controller = application.getController('App.controller.kylw.ArrangeTeckyController');
...
detail : function(o, e, eOpts){
	...
	tjedit : function(o, e, eOpts) {
		...
		Ext.MessageBox.alert('提示', '请选择一条数据记录！');
			...
			var shjg = "";
			Ext.Ajax.request({
						url : 'xwKylwbchangeFlowFlag.action',
						method : 'get',
						params : {
							xh : xh,
							kyuuid : kyuuid,
						...
						success : function(response) {
							...
							var viewXwKylwzbLists = Ext.ComponentQuery
									.query('#viewXwKylwzbList');
							if (viewXwKylwzbLists.length > 0) {
								var viewXwKylwzbList = viewXwKylwzbLists[0];
								var store = viewXwKylwzbList.getStore();
								store.load();
							}
						},
...
//			},
	...
	search : function(o, e, eOpts) {
		...
		var pGrid = tab.down('#viewXwKylwzbList'), store = me
				.getStore('ViewXwKylwzbStore');
...
//				});
	...
	searchAll : function(o, e, eOpts) {
		...
		var searchForm = tab.down('#searchForm');
		var pGrid = tab.down('#viewXwKylwzbList');
