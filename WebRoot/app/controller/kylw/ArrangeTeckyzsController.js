Ext.define('App.controller.kylw.ArrangeTeckyzsController',{
	extend:'Ext.app.Controller',
	
	models : [ 'xs.XsJcxxbModel',
			'kylw.XwKylwzlbModel','kylw.XwKylwbModel'],
	stores : ['kylw.XwKylwzlbStore','kylw.XwKylwbStore','zd.ZdShjgmStore'],
//	refs: [{  
...
//    }],
	...
	init:function(){
		this.control({
				...
				'#arrangeTecWindowjlbzs  button[action=save]':{
					click:this.addJs
				}
			});
		},
		addJs:function(o, e, eOpts){
			...
			var kkjhAddForm = win.down('#kkjhAddzsForm');
			var arrangeTecshzsForm = kkjhAddForm.down('#arrangeTecshzsForm');//审核
			var arrangeTecshkyzsForm = kkjhAddForm.down('#arrangeTecshkyzsForm');//科研
			...
			var xwKtbgsqb2 = arrangeTecshkyzsForm.down('#viewXwDbmsb1');
			var json = {}
...
//					    }  
  		...
  		/*detailJs:function(o, e, eOpts){
			...
			onLaunch : function(o,skxxList,kyuuid) {
				...
				var win = Ext.create('App.view.kylw.ArrangeTeckyzsWindow',{
						skxxList:skxxList,
						kyuuid:kyuuid,
						x : 300,
						y : 3,
...
//				win.down('#arrangeTecshForm').loadForm(recs[0])
				win.down('#arrangeTecshkyzsForm').loadForm(recs[0])
				win.down('#arrangeTecjbForm').loadForm(recs[0])
				win.down('#arrangeTecshkyzsForm').setViewdForm();
				win.down('#arrangeTecjbForm').setViewForm();
				var xwKylwzlbStore = win.down('#xwKylwzlbList').getStore('XwKylwzlbStore');
				var xwKylwzlbProxy = xwKylwzlbStore.getProxy();
				var searchParams = {
					searchMode : 'simple',
					search : {}
				};
				searchParams.search['kyuuid'] = "#= '" + kyuuid + "'";
				xwKylwzlbProxy.setExtraParam('params', Ext.JSON.encode(searchParams));
//									    	   var store = win.xwKylwzlbList.getStore();
			    	   xwKylwzlbStore.load();
				win.show();
			}
	})
