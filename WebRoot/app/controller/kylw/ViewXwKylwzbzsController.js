Ext.define('App.controller.kylw.ViewXwKylwzbzsController', {
	...
	refs : [{
				selector : 'viewXwKylwzbzsSearchForm',
				ref : 'viewXwKylwzbzsSearchForm'
			}],
	...
	init : function() {
		this.control({
					...
					'#viewXwKylwzbzsList gridtoolbar combo[itemId=numCmb]' : {
						render : this.initPageSize,
						select : this.setPageSize
					},
					'#viewXwKylwzbzsList gridtoolbar button[action=detail]' : {
						click : this.detail
					},
					'#viewXwKylwzbzsList  button[action=tjedit]' : {
						click : this.tjedit
					},
					'#viewXwKylwzbzsList gridtoolbar button[action=editsj]' : {
						click : this.editClass
					},
					'#viewXwKylwzbzsList gridtoolbar button[itemId=schShowBtn]' : {
						render : this.setSchShowText
					},

					'#viewXwKylwzbzsList gridtoolbar button[action=showSearch]' : {
	...
	onLaunch : function(record) {
		...
		var contentPanel = Ext.create(
				'App.view.kylw.ViewXwKylwzbzsContentPanel', {
					itemId : 'content',
	...
	editClass : function(o, e, eOpts){
					...
					var skxxList = o.up('#viewXwKylwzbzsList')
					var application = me.getApplication();
	        		var controller = application.getController('App.controller.kylw.ArrangeTeckyzsController');
...
detail : function(o, e, eOpts){
					...
					var skxxList = o.up('#viewXwKylwzbzsList')
					var application = me.getApplication();
	        		var controller = application.getController('App.controller.kylw.ArrangeTeckyxqzsController');
			  	    controller.onLaunch(o,skxxList,kyuuid);
					}
			},
	...
	tjedit : function(o, e, eOpts) {
		...
		Ext.MessageBox.alert('提示', '请选择一条数据记录！');
			...
			var shjg = "";
			Ext.Ajax.request({
						url : 'xwKylwbchangeFlowFlagg.action',
						method : 'get',
						params : {
							xh : xh,
							kyuuid : kyuuid,
						...
						success : function(response) {
							...
							var viewXwKylwzbzsLists = Ext.ComponentQuery
									.query('#viewXwKylwzbzsList');
							if (viewXwKylwzbzsLists.length > 0) {
								var viewXwKylwzbzsList = viewXwKylwzbzsLists[0];
								var store = viewXwKylwzbzsList.getStore();
								store.load();
							}
						},
	...
	search : function(o, e, eOpts) {
		...
		var pGrid = tab.down('#viewXwKylwzbzsList'), store = me
...
//				});
	...
	searchAll : function(o, e, eOpts) {
		...
		var searchForm = tab.down('#searchForm');
		var pGrid = tab.down('#viewXwKylwzbzsList');
