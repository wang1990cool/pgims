Ext.define('App.controller.kyzz.ArrangeTeczzzsController',{
...
//    }],
	...
	init:function(){
		this.control({
				...
				'#arrangeTecWindowjlbzzzs  button[action=save]':{
					click:this.addJs
				}
			});
		},
		addJs:function(o, e, eOpts){
			...
			var arrangeTecshzzzsForm = kkjhAddForm.down('#arrangeTecshzzzsForm');//科研
			...
			var xwKtbgsqb2 = arrangeTecshzzzsForm.down('#viewXwDbmsb1');
			var json = {}
			var items = xwKtbgsqb2.items;
			values = arrangeTecshzzzsForm.getValues();
...
//					    }  
  		...
  		/*detailJs:function(o, e, eOpts){
			...
			onLaunch : function(o,skxxList,kyuuid) {
				...
				var win = Ext.create('App.view.kyzz.ArrangeTeczzzsWindow',{
						skxxList:skxxList,
						kyuuid:kyuuid,
						x : 300,
						y : 3,
...
//				win.down('#arrangeTecshForm').loadForm(recs[0])
				win.down('#arrangeTecshzzzsForm').loadForm(recs[0])
				win.down('#arrangeTecjbForm').loadForm(recs[0])
				win.down('#arrangeTecshzzzsForm').setViewdForm();
