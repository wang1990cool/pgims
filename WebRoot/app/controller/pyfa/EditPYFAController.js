Ext.define('App.controller.pyfa.EditPYFAController',{
	...
	refs : [{
		selector : 'editPyfaWin',
		ref : 'editPyfaWin'
	}],
	
	init:function(){
			this.control({
				'editPyfaWin button[action=add]':{
					click: this.addFAKC
				},
				'editPyfaWin button[action=delete]':{
					click:this.deleteFAKC
				},
				'editPyfaWin button[action=detail]':{
					click:this.detailFAKC
				},
				'editPyfaWin button[action=edit]':{
					click:this.editFAKC
				},
				'editPyfaWin button[action=save]':{
					click:this.savePYFA
				},
				'editPyfaWin button[action=ok]':{
					click:this.submitPYFA
				},
				'editPyfaWin button[action=uploadAttach]':{
					click:this.uploadAttach
				},
				'editPyfaWin button[action=delAttach]':{
					click:this.delAttach
				},
				'pyfaDialog button[action=pyfaDetail]':{
					click:this.pyfaDetail
				},
				'editPyfaWin button[action=import]':{
					click:this.importPyfa
				},
				'pyfaDialog button[action=pyfaDetail]':{
					click:this.pyfaDetail
				},
				'pyfaDialog button[action=importFakc]':{
					click:this.importFakc
				}
			});
		},
	...
	uploadAttach:function(o, e,eOpts){
		...
		savePYFA:function(o, e, eOpts ){
			var me = this;
			var win = o.up('window');
			var pyfaEditForm = win.down('#pyfaEditForm');
			var pyfaForm = pyfaEditForm.down('#pyfaForm');
			var tab = pyfaEditForm.getActiveTab();
			
			if(tab == pyfaEditForm.child('#pyfaFormTab')){
				me.savePyfa(pyfaForm);// 保存培养方案
			}
			else if(tab == pyfaEditForm.child('#fakcListTab')){
				me.saveFakc(pyfaForm);// 保存方案课程
			}else if(tab == pyfaEditForm.child('#attachDataTab')){
				me.saveAttach(pyfaEditForm);
			}
		},
		
		saveAttach:function(pyfaEditForm){
			var me = this;
			var attachList = pyfaEditForm.down('#uploadAttachList');
			var pyfaForm = pyfaEditForm.down('#pyfaForm');
			var pyfahValue = pyfaForm.down('#pyfah').getValue();
		...
		jsonParse :function(record){
		 ...
		 saveFakc:function(pyfaForm){
			var me = this;
			var pyfahValue = pyfaForm.down('#pyfah').getValue();
			var fakcListTab = pyfaForm.up('#pyfaEditForm').down('#fakcListTab');
			...
			for (var i = 0; i < fakcStore.getCount(); i++) { 
				...
				if(record.data.kclb == '' || record.data.kcxz == '' || record.data.kcsx == ''
					...
					Ext.MessageBox.alert('提示','请将课程类别、课程性质、课程属性、' +
							'考试形式和开课学期填写完整！');
					flag = false;
				}
				var jsonObject = me.jsonParse(record);
			...
			if(flag){
				Ext.Ajax.request({
						...
						waitTitle : '提示',
						method : 'post',
						params:{datas:JSONList,pyfaDatas:JSONobj},
						waitMsg : '正在提交数据请稍候...',
						success : function(response, opts) {
							var result = Ext.decode(response.responseText);
							var success = result.result.success;
...
//	    },
	...
	//			        		  me.up('window').close();
		...
		submitPYFA:function(o, e, eOpts){
			var me = this;
			var win = o.up('window');
			var fakcListTab = win.down('#pyfaEditForm').down('#fakcListTab');
			
			var pyfaFormTab = win.down('#pyfaEditForm').down('#pyfaFormTab');
	        ...
	        proxy.setExtraParam('params', Ext.JSON.encode(searchParams));
		    fakcStore.load({callback:function(){
		        var JSONList = [];
				//获得Store中的数据
				var flag = true;
				for (var i = 0; i < fakcStore.getCount(); i++) { 
		...
		addFAKC:function(o, e, eOpts){
			...
			deleteFAKC:function(o, e, eOpts){
				...
				if(recs.length == 0){
					...
					var fakcList = o.up('#fakcListTab').down('#fakcList');
...
//			deleteFAKC:function(o, e, eOpts){
//    			var action = 'fakcDel.action';
//    			var fakcGrid = o.up('#pyfaEditForm').down('#fakcList');
//    			var ids = getSelIds(fakcGrid,'id');
//    			if(ids.length ==0) return;
//				var store = fakcGrid.getStore('FAKCStore');
...
//							})
				 			 var editPyfaWin = Ext.create('App.view.pyfa.EditPYFAWindow',{
				 			 	itemId:'editPyfaWin'
				 			 });
				 			 
				 			 var pyfaForm = editPyfaWin.down('#pyfaForm')
					 		pyfaForm.down('#zymc').setVisible(true)
				           
				        	pyfaForm.isAdd = false;
//				        	pyfaForm.down('#bbh').setReadOnly(true);
//				        	pyfaForm.down('#dwmc').setReadOnly(true);
//				        	pyfaForm.down('#zydm').setReadOnly(true);
//				        	pyfaForm.down('#pyfah').setReadOnly(true);
//				        	pyfaForm.down('#xslx').setReadOnly(true);
				        	var textfields =  pyfaForm.query('textfield');
							for(var i in textfields){
								if(	textfields[i].itemId == 'bbh' || 	textfields[i].itemId == 'dwmc'  || 	textfields[i].itemId == 'zymc' 
									||	textfields[i].itemId == 'pyfah' || textfields[i].itemId == 'xslx'){
