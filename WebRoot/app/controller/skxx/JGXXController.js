Ext.define('App.controller.skxx.JGXXController',{
				...
				if (recs.length == 0) {
					...
					if(jslxValue == 'zjjs' ){
						if(recs.length > 1){
							...
							form.down('#zjjsh').setValue(recs[0].data.gh);
					    	form.down('#zjjs').setValue(recs[0].data.xm);
						...
						form.down('#zkjsh').setValue(zkjsh);
					    form.down('#zkjs').setValue(zkjs);
					    o.up('#jgxxDialogWin').close();
						...
						form.down('#syjsh').setValue(syjsh);
					    form.down('#syjs').setValue(syjs);
					    o.up('#jgxxDialogWin').close();
					}
				}
          },
  		...
  		detailJs:function(o, e, eOpts){
			...
			onLaunch : function(o,jslx,form) {
					var win = new Ext.Window({
						...
						jslx:jslx,
						items:[
						Ext.create('App.view.skxx.JGXXDialog',{
							itemId:'jgxxDialog'
						})]
					});
