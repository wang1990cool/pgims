Ext.define('App.controller.psdm.PsdmController', {
...
//    }],
   		...
   		init: function() {
		this.control({
			'psdmList gridtoolbar combo[itemId=numCmb]': {
					render: this.initPageSize,
					select: this.setPageSize
			},
			
			
			'psdmList button[itemId=schShowBtn]' : {
					render : this.setSchShowText
			       },
		    'psdmList button[action=showSearch]' : {
				    click : this.showSearch
				   },
			
			'psdmList button[action=detail]':{
				click:this.detailClass
					},
			
					
			'psdmList button[action=printPDF]': {
				click: this.printPDF
			},
    ...
    showSearch: function( o, e, eOpts){
     ...
     search: function( o, e, eOpts){
		...
		var pGrid = tab.down('#psdmList'), store = pGrid.getStore();
...
//		    		url:'psdmtoPDF.action?kkkh=' + kkkh,
	 ...
	 initStore : function() {
		...
		onLaunch : function(record) {
				...
				var contentPanel = Ext.create('App.view.psdm.PsdmContentPanel',
						{
							itemId : 'content',
