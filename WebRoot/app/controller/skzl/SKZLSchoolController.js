Ext.define('App.controller.skzl.SKZLSchoolController', {
			...
			init : function() {
				this.control({
							'skzlSubmitList combo[itemId=numCmb]': {
								render: this.initPageSize,
								select: this.setPageSize
							},
						  'skzlSubmitList button[itemId=schShowBtn]' : {
							render : this.setSchShowText
						   },
						   'skzlSubmitList button[action=showSearch]' : {
							      click : this.showSearch
							},
							'skzlSubmitList  button[action=detail]':{
								click: this.detailSkzl
							},
							'skzlSubmitList  button[action=withdraw]':{
...
//				wjlxStore.load();
			...
			onLaunch : function(record) {
				...
				var contentPanel = Ext.create('App.view.skzl.SKZLSubmitContentPanel',
					{
						itemId : 'content',
