Ext.define('App.controller.cjlr.CjqrController', {
    extend: 'Ext.app.Controller',
    
    models: ['cjlr.KcztModel','cjlr.JxCjmxbModel'],
    stores: ['cjlr.CjqrStore','cjlr.JxCjmxbStore'],
    views:  ['cjlr.CjqrConPan'],
//        refs: [{  
//        selector: 'kcxxForm',  
//        ref: 'kcxxForm'  
//    }],
...
//			}
			...
			'cjqrList gridtoolbar combo[itemId=numCmb]':{
				render: this.initPageSize,
				select: this.setPageSize
			},
			
			'cjqrList gridtoolbar button[itemId=schShowBtn]': {
				render: this.setSchShowText
			},
			
			'cjqrList gridtoolbar button[action=showSearch]': {
				click: this.showSearch
			},
			
			'cjqrList button[action=display]':{
				click:this.display
			},
			
			'cjqrList button[action=reback]':{
				click:this.reback
			},
    ...
    onLaunch : function(record) {
				...
				var contentPanel = Ext.create('App.view.cjlr.CjqrConPan',
						{
							itemId : 'content',
...
//						    	iconCls:'topArrow',
//								tooltip: '通过确认',
//							    hidden: (roleCode == 'GSJX'?false:true),
//								handler:function(){
//									if(sjzt == '2'){
//										Ext.Msg.show({title:'提示', msg:'本门课程信息已经通过确认！无法再次确认',buttons: Ext.Msg.OK,icon: Ext.Msg.WARNING});
//										return false;
//									}
//									this.up('window').down('#disCjlrPan').pass(kkkh);
//							}
//							},'-',
							{					
							text: '关闭',
...
//			    			kcxxForm.down('#ksxs').setValue(cjjlData.ksxs);
    ...
    setPageSize: function(o, e, eOpts){
     ...
     search: function( o, e, eOpts){
		...
		var pGrid = tab.down('#cjqrList'), store = me.getStore('cjqrStore');
...
//			searchParams.search['isPass'] = "#= " + isValid;
    ...
    searchAll: function( o, e, eOpts){
    	...
    	var store = me.getStore('cjqrStore'), proxy = store.getProxy();
