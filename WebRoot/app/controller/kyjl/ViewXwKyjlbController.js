Ext.define('App.controller.kyjl.ViewXwKyjlbController', {
	extend : 'Ext.app.Controller',

	models : ['kyjl.ViewXwKyjlbModel', 'xs.XsJcxxbModel','kyjl.XwKyjlbModel','kyjl.XwKyjlzlbModel'],
	stores : ['zd.ZdFwqbStore','kyjl.ViewXwKyjlbStore','zd.ZdSztmStore','xs.XsJcxxbStore','kyjl.XwKyjlbStore','kyjl.XwKyjlzlbStore'],

	refs : [{
				selector : 'viewXwKyjlbSearchForm',
				ref : 'viewXwKyjlbSearchForm'
			}],
	...
	init : function() {
		this.control({
					...
					'#viewXwKyjlbList gridtoolbar combo[itemId=numCmb]' : {
						render : this.initPageSize,
						select : this.setPageSize
					},
					'#viewXwKyjlbList gridtoolbar button[action=detail]' : {
						click : this.detail
					},
					'#viewXwKyjlbList  button[action=add]' : {
						click : this.Add
					},
					'#viewXwKyjlbList  button[action=tjedit]' : {
						click : this.tjedit
					},
					'#viewXwKyjlbList  button[action=tjchedit]' : {
						click : this.tjchedit
					},
					'#xwLwkyjlzlbList  button[action=zlUpload]' : {
						click : this.zlUpload
					},
					'#viewXwKyjlbList button[action=delete]' : {
						click : this.del
					},
					'#viewXwKyjlbList gridtoolbar button[action=editsj]' : {
						click : this.editClass
					},
					'#viewXwKyjlbList gridtoolbar button[action=delete]' : {
						click : this.deletexx
					},
					 '#viewXwKyjlbList gridtoolbar button[action=searchXh]': {
					 click : this.searchXh
					 },
					'#viewXwKyjlbList gridtoolbar button[itemId=schShowBtn]' : {
						render : this.setSchShowText
					},

					'#viewXwKyjlbList gridtoolbar button[action=showSearch]' : {
	...
	initStore : function() {
		var me = this;
		var store = me.getStore('ViewXwKyjlbStore', {});
		var searchParams = {
			start : 0,
			limit : 15,
			page : 1,
	...
	excelBtnRender : function(o, e, eOpts) {
		o.actionUrl = 'viewXwKyjlbtoExcel.action'
	},
	...
	onLaunch : function(record) {
		...
		var contentPanel = Ext.create(
				'App.view.kyjl.ViewXwKyjlbContentPanel', {
					itemId : 'content',
	...
	/*detail : function(o, e, eOpts) {
		...
		if(rec == undefined){Ext.MessageBox.alert('提示', '请选择一条数据记录！');}
			...
			var win = new Ext.Window({
						...
						animCollapse : true,
						autoShow : true,
						items : [Ext.create('App.view.kyjl.ViewXwKyjlbForm')]
					});
			var viewXwKyjlbshForm = win.down('#viewXwKyjlbForm');
			viewXwKyjlbshForm.loadForm(rec)
			 var sj = rec.data.lrsj;
			 var yearsj = sj.substring(0,10);
			 var monthsj = sj.substring(11,19);
			 lrsj = yearsj+ " " +monthsj;
			viewXwKyjlbshForm.down('#lrsj').setValue(lrsj);
			
		}
		var textfields = viewXwKyjlbshForm.query('textfield');
		...
		var me = this;
		var xh = rec.data.xh
		var xwKtbgzlbStore = me.getStore('XwLwkyjlzlbStore');
	...
	Add:function(o, e, eOpts){
					var me = this;
					var skxxList = o.up('#viewXwKyjlbList')
					var application = me.getApplication();
	        		var controller = application.getController('App.controller.kyjl.ArrangeTecController');
	...
	editClass : function(o, e, eOpts){
					...
					var skxxList = o.up('#viewXwKyjlbList')
					var application = me.getApplication();
	        		var controller = application.getController('App.controller.kyjl.ArrangeTecjlController');
...
detail : function(o, e, eOpts){
	...
	tjedit : function(o, e, eOpts) {
		...
		Ext.MessageBox.alert('提示', '请选择一条数据记录！');
			...
			var shjg = "";
			Ext.Ajax.request({
						url : 'xwKyjlbchangeFlowFlag.action',
						method : 'get',
						params : {
							xh : xh,
							kyuuid : kyuuid,
						...
						success : function(response) {
							...
							var viewXwKyjlbLists = Ext.ComponentQuery
									.query('#viewXwKyjlbList');
							if (viewXwKyjlbLists.length > 0) {
								var viewXwKyjlbList = viewXwKyjlbLists[0];
								var store = viewXwKyjlbList.getStore();
								store.load();
							}
						},
...
//			},
	...
	search : function(o, e, eOpts) {
		...
		var pGrid = tab.down('#viewXwKyjlbList'), store = me
				.getStore('ViewXwKyjlbStore');
...
//				});
	...
	searchAll : function(o, e, eOpts) {
		...
		var searchForm = tab.down('#searchForm');
		var pGrid = tab.down('#viewXwKyjlbList');
