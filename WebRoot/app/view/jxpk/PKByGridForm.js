Ext.define('App.view.jxpk.PKByGridForm',{
	extend:'Ext.form.Panel',
	alias : 'widget.pkByGridForm',
	xtype : 'form',
			     ...
			     width:345,
				   defaults:{
					...
					items:[{
						...
						itemId:'zjjs',
						name:'zjjs',
						fieldLabel:'<font color="red">*</font>主讲教师',
						blankText : '必填！',
					    width:300,
					  	readOnly:true,
						labelWidth:100,
			      ...
			      width:345,
				   ...
				   defaults:{
					...
					items:[{
						...
						labelWidth:100,
						listeners:{
							change:function(text,newVal,oldVal,eOpts){
								if(newVal.length != 0)
									me.setOccupied();
							}
						}
...
//												me.down('#jszCombo').setValue(null);
      ...
      setViewForm :function(){
    	...
    	var textfields =  me.query('textfield');
    		for(var i in textfields){
    				if( textfields[i].itemId == 'kch' ||  textfields[i].itemId == 'kcmc' || textfields[i].itemId == 'pydw' 
    					|| textfields[i].itemId == 'zymc' || textfields[i].itemId == 'ckjs' || textfields[i].itemId == 'cksj'){
     ...
     setPksj: function(){
		...
		me.setKcsjm(pkGrid.getWeekNum())   	    	
		var darr = [], oarr = pkGrid.getJc();
    	for(var i = 0; i < oarr.length; i++){
    		if(oarr[i]) darr.push(oarr[i]);
  		}
		me.setKcjc(darr.join('/'));
    ...
    setPkGrid:function(){
    	var me = this;
    	var jsValue = me.down('#jsmc').getValue();
    ...
    setPkxx: function(){
    	...
    	var sjzxapbz = me.down('#sjzxapbz').getValue();
//    	if(!sjzxapbz){
	    	var kczs = me.getKczs();
	    	if(kczs){
...
//    	}
    ...
    setOccupied:function(){
    	 var me = this;
    	var jsmc = me.down('#jsmc').getValue();
    	var jsbh = me.down('#jsbh').getValue();
    	var lxz = me.down('#lxzRadio').getValue();
		var tz = me.down('#tzRadio').getValue();
    	if(lxz){
    		var kszValue = me.down('#kszCombo').getValue();
   			var jszValue = me.down('#jszCombo').getValue();
    		if(kszValue != null && jszValue != null && jsmc.length != 0){
    			var kczs = '';
    			var kszValue = Number(me.down('#kszCombo').getValue());
	   			var jszValue = Number(me.down('#jszCombo').getValue());
				for(var i = kszValue; i <= jszValue; i++){
				...
				kczs = kczs.substring(0,kczs.length - 1);
				me.getOccupied(kczs,jsbh)
    		}
    	}else{
			var kczsValue = me.down('#kczs').getValue();
			if(kczsValue != undefined && jsmc.length != 0){
    ...
    getOccupied:function(kczs){
    	var me = this;
    	var win = me.up('window');
    	var pkGrid = win.down('#pkGrid');
    	
  		var xn = me.down('#xn').getValue();
    	var xq = me.down('#xq').getValue()
    	var jsbh = win.down('#jsbh').getValue();
    	var ggkbzm = win.down('#ggkbzm').getValue();
    	var kkdwh =  win.down('#kkdwh').getValue();
...
//					pkGrid.orgPkStore =  pkGrid.getStore();
