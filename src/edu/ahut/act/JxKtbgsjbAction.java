...
public class JxKtbgsjbAction extends ActionSupport{

	private static final long serialVersionUID = 4183786154194055246L;
	private String filter = null;
	private List<?> datas;
	private JxKtbgsjbService jxKtbgsjbService;
...
//	private String msg;
	...
	private String sznj;
	private String xwbz;
	public JxKtbgsjbAction() {
		jxKtbgsjbService = (JxKtbgsjbService) SpringContextUtil
				.getBean("JxKtbgsjbServiceImpl");
	}
	
	public JxKtbgsjbService getJxSkxxbService() {
		return jxKtbgsjbService;
	}

	public void setJxKtbgsjbService(JxKtbgsjbService jxKtbgsjbService) {
		this.jxKtbgsjbService = jxKtbgsjbService;
	}
...
//	}
		...
		try {
			JSONUtils.getMorpherRegistry().registerMorpher(
					new DateMorpher(new String[] { "yyyy-MM-dd",
							"yyyy-MM-dd HH:mm:ss" }));
			JxKtbgsjb jxKtbgsjb = (JxKtbgsjb) JSONObject.toBean(json, JxKtbgsjb.class);
			jxKtbgsjbService.add(jxKtbgsjb);
		...
		try {
			JSONUtils.getMorpherRegistry().registerMorpher(
					new DateMorpher(new String[] { "yyyy-MM-dd",
							"yyyy-MM-dd HH:mm:ss" }));
			JxKtbgsjb jxKtbgsjb = (JxKtbgsjb) JSONObject.toBean(json, JxKtbgsjb.class);
			jxKtbgsjbService.update(jxKtbgsjb);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
	...
	public String Del() {
		try {
			...
			ids = "'" + ids.replace(",","','") + "'";
			String hql ="delete JxKtbgsjb where id in (" + ids + ")"; 
			jxKtbgsjbService.hqlExcute(hql);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
	...
	public String List() {
		...
		Page<JxKtbgsjb> pageList = jxKtbgsjbService.pageQuery(startNo,
	...
	public String toExcel(){
		...
		if(!excelParams.equals("")){
			...
			try {
				...
				List<?> tList = jxKtbgsjbService.Query(filters, order);
	...
	public String getSj(){
		try{
			...
			String order = null;
			List<JxKtbgsjb> list =  (List<JxKtbgsjb>)this.jxKtbgsjbService.Query(filter,order);
...
//					}
	...
	public String changeFlowFlag() {
		try{
			jxKtbgsjbService.changeFlowFlag(sznj,xn,xq,fyh,zydm,xslxm,xwbz);
			result.setSuccess(true);
		}catch(Exception e){
			result.setSuccess(false);
