...
public class XwKylwbAction extends ActionSupport{

	private static final long serialVersionUID = 4183786154194055246L;
	private String filter = null;
	private List<?> datas;
	private XwKylwbService xwKylwbService;
...
//	private String shjg;
	private Date zssj;
	public XwKylwbAction() {
		xwKylwbService = (XwKylwbService) SpringContextUtil
				.getBean("XwKylwbServiceImpl");
		viewXwKylwzbService = (ViewXwKylwzbService) SpringContextUtil
				.getBean("ViewXwKylwzbServiceImpl");
	}
	
	public XwKylwbService getJxSkxxbService() {
		return xwKylwbService;
	}

	public void setXwKylwbService(XwKylwbService xwKylwbService) {
		this.xwKylwbService = xwKylwbService;
	}
...
//	}
		...
		try {
			JSONUtils.getMorpherRegistry().registerMorpher(
					new DateMorpher(new String[] { "yyyy-MM-dd",
							"yyyy-MM-dd HH:mm:ss" }));
			XwKylwb xwKylwb = (XwKylwb) JSONObject.toBean(json, XwKylwb.class);
			xwKylwbService.add(xwKylwb);
	...
	public String Del() {
		try {
			...
			ids = "'" + ids.replace(",","','") + "'";
			String hql ="delete XwKylwb where id in (" + ids + ")"; 
			xwKylwbService.hqlExcute(hql);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
	...
	public String Del1(){
		try {
			String ids = ServletActionContext.getRequest().getParameter("ids");
			xwKylwbService.deleteByIds(ids);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
...
//		filter = " (fyh='" + szdwh + "') ";
		...
		Page<XwKylwb> pageList = xwKylwbService.pageQuery(startNo,
	...
	public String toExcel(){
		...
		if(!excelParams.equals("")){
			...
			try {
				...
				List<?> tList = xwKylwbService.Query(filters, order);
...
//			String order = null;
//			List<XwKylwb> list =  (List<XwKylwb>)this.xwKylwbService.Query(filter,order);
//			
//			Date date = new Date();
//			
...
//	}
	public String changeFlowFlag() {
		try{
			xwKylwbService.changeFlowFlag(xh,kyuuid,ztm,zt,shrgh,shr,shsj,shyj,shjgm,shjg);
			result.setSuccess(true);
		}catch(Exception e){
			result.setSuccess(false);
	...
	public String changeFlowFlagg() {
		try{
			xwKylwbService.changeFlowFlagg(xh,kyuuid,ztm,zt,zsrgh,zsr,zssj,zsyj,shjgm,shjg);
			result.setSuccess(true);
		}catch(Exception e){
			result.setSuccess(false);
			result.setMsg("读取数据失败！");
	...
	public String GetKYUUID(){
		try{
			...
			setKyuuid(this.xwKylwbService.getKyuuid(KYUUID));
			result.setSuccess(true);
		}catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
