...
public class ViewXwKyjlbAction extends ActionSupport{
	private static final long serialVersionUID = 8411848302070961482L;

	static private String filter = null;
	private List<?> datas;
	private ViewXwKyjlbService viewXwKyjlbService;
	private XwKtbgsqbService xwKtbgsqbService;
	private JsonResult result = new JsonResult();
	
	public ViewXwKyjlbAction(){
		viewXwKyjlbService = (ViewXwKyjlbService) SpringContextUtil.getBean("ViewXwKyjlbServiceImpl");
	}
	...
	public ViewXwKyjlbService getViewXwKyjlbService() {
		return viewXwKyjlbService;
	}

	public void setViewXwKyjlbService(ViewXwKyjlbService viewXwKyjlbService) {
		this.viewXwKyjlbService = viewXwKyjlbService;
	}
...
//		filter = "xh = '" + xh + "'";
		...
		Page<ViewXwKyjlb> pageList = viewXwKyjlbService.pageQuery(startNo, pageSize, filters, orders);
...
//		filter = "xh = '" + xh + "'";
	...
	public String Add(){
		...
		try {
			ViewXwKyjlb pyKck = (ViewXwKyjlb) JSONObject.toBean(json, ViewXwKyjlb.class);
			viewXwKyjlbService.add(pyKck);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
...
//			System.out.println(pyKck);
	...
	public String Del(){
		try {
			...
			viewXwKyjlbService.deleteByIds(ids);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
	...
	public String CheckByXh(){
		try {
			...
			boolean isExist = this.viewXwKyjlbService.checkIsExistByXh(xh);
	...
	public String toExcel(){
		...
		if(!excelParams.equals("")){
			...
			try {
				...
				List<?> tList = viewXwKyjlbService.Query(filters, order);
