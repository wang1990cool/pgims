...
public class JxZxkkjhbAction extends ActionSupport{
	private static final long serialVersionUID = 5753359956958912148L;
	
    private static String filter = "";
	private List<?> datas;
	private List<?> skjhDatas;
	private JxZxkkjhbService jxZxkkjhbService;
	...
	public JxZxkkjhbAction(){
		jxZxkkjhbService = (JxZxkkjhbService) SpringContextUtil.getBean("JxZxkkjhbServiceImpl");
	...
	public  String getFilter() {
		...
		if(skjhList.size() > 0){
			for(int i = 0;i < skjhList.size();i++){
				conditon.append("(kkjhh <>'" + skjhList.get(i).getKkjhh() + "' and kch <>'" +  skjhList.get(i).getKch() + "')");
				if((i+1) != skjhList.size() )
					conditon.append(" or ");
			}
		}
		filter = conditon.toString();
		return filter;
	}

	public static void setFilter(String filter) {
		JxZxkkjhbAction.filter = filter;
	}
	...
	public JxZxkkjhbService getJxZxkkjhbService() {
		return jxZxkkjhbService;
	}

	public void setJxZxkkjhbService(JxZxkkjhbService jxZxkkjhbService) {
		this.jxZxkkjhbService = jxZxkkjhbService;
	}
	...
	public String List(){
		...
		Page<JxZxkkjhb> pageList = jxZxkkjhbService.pageQuery(startNo, pageSize, filters, orders);
	...
	public String toExcel(){
		...
		if(!excelParams.equals("")){
			...
			try {
				...
				List<?> tList = jxZxkkjhbService.Query(filters, order);
...
//	}
	...
	public String ListSkjh(){
		try {
			List<?> list = jxZxkkjhbService.getAll();
