...
public class ViewXwKyzzbAction extends ActionSupport{
	private static final long serialVersionUID = 8411848302070961482L;

	static private String filter = null;
	private List<?> datas;
	private ViewXwKyzzbService viewXwKyzzbService;
	private XwKtbgsqbService xwKtbgsqbService;
	private JsonResult result = new JsonResult();
	
	public ViewXwKyzzbAction(){
		viewXwKyzzbService = (ViewXwKyzzbService) SpringContextUtil.getBean("ViewXwKyzzbServiceImpl");
	}
	...
	public ViewXwKyzzbService getViewXwKyzzbService() {
		return viewXwKyzzbService;
	}

	public void setViewXwKyzzbService(ViewXwKyzzbService viewXwKyzzbService) {
		this.viewXwKyzzbService = viewXwKyzzbService;
	}
...
//		filter = "xh = '" + xh + "'";
		...
		Page<ViewXwKyzzb> pageList = viewXwKyzzbService.pageQuery(startNo, pageSize, filters, orders);
...
//		filter = "xh = '" + xh + "'";
	...
	public String Add(){
		...
		try {
			ViewXwKyzzb pyKck = (ViewXwKyzzb) JSONObject.toBean(json, ViewXwKyzzb.class);
			viewXwKyzzbService.add(pyKck);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
...
//			System.out.println(pyKck);
	...
	public String Del(){
		try {
			...
			viewXwKyzzbService.deleteByIds(ids);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
	...
	public String CheckByXh(){
		try {
			...
			boolean isExist = this.viewXwKyzzbService.checkIsExistByXh(xh);
	...
	public String toExcel(){
		...
		if(!excelParams.equals("")){
			...
			try {
				...
				List<?> tList = viewXwKyzzbService.Query(filters, order);
