...
public class XwKylwzlbAction extends ActionSupport{
...
//	private ViewXwKtbgsqService viewXwKtbgsqService;
	...
	public XwKylwzlbAction() {
		xwKylwzlbService = (XwKylwzlbService) SpringContextUtil
				.getBean("XwKylwzlbServiceImpl");
...
//	}
	...
	public String Add(){
		...
		try {
			XwKylwzlb xwKylwzlb = (XwKylwzlb) JSONObject.toBean(json, XwKylwzlb.class);
			xwKylwzlbService.add(xwKylwzlb);
	...
	public String Edit() {
		...
		try {
			XwKylwzlb xwKylwzlb = (XwKylwzlb) JSONObject.toBean(json, XwKylwzlb.class);
			xwKylwzlbService.update(xwKylwzlb);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
...
//			xwKylwzlbService.hqlExcute(hql);
//			result.setSuccess(true);
//		} catch (Exception e) {
//			e.printStackTrace();
//			result.setSuccess(false);
...
//		filter = "xh = '" + xh + "'";
		...
		String filters = CommonUtil.MergeFilter(filter, searchParams);
		Page<XwKylwzlb> pageList = xwKylwzlbService.pageQuery(startNo, pageSize, filters, orders);
		setResult(new JsonResult(pageList.getResult(), pageList.getTotalCount()));
		result.setSuccess(true);
		return SUCCESS;
	}
	...
	public String toExcel(){
		...
		if(!excelParams.equals("")){
			...
			try {
				...
				List<?> tList = xwKylwzlbService.Query(filters, order);
	...
	public String Del(){
		try {
			String ids = ServletActionContext.getRequest().getParameter("ids");
			xwKylwzlbService.deleteByIds(ids);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
	...
	public String Del1(){
		try {
			String ids = ServletActionContext.getRequest().getParameter("ids");
			xwKylwzlbService.deleteByIds1(ids);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
	...
	public String FileUploadKy() {
		System.out.println("FileUploadKy");
		return "pfileUploadKy";
	}
	
	public String FileDeleteKy() {
		return "pfileDeleteKy";
	}
	
	public String FileDownloadKy() {
		return "pfileDownloadKy";
	}
