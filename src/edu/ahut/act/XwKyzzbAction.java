...
public class XwKyzzbAction extends ActionSupport{

	private static final long serialVersionUID = 4183786154194055246L;
	private String filter = null;
	private List<?> datas;
	private XwKyzzbService xwKyzzbService;
...
//	private String shjg;
	private Date zssj;
	public XwKyzzbAction() {
		xwKyzzbService = (XwKyzzbService) SpringContextUtil
				.getBean("XwKyzzbServiceImpl");
		viewXwKylwzbService = (ViewXwKylwzbService) SpringContextUtil
				.getBean("ViewXwKylwzbServiceImpl");
	}
	
	public XwKyzzbService getJxSkxxbService() {
		return xwKyzzbService;
	}

	public void setXwKyzzbService(XwKyzzbService xwKyzzbService) {
		this.xwKyzzbService = xwKyzzbService;
	}
...
//	}
		...
		try {
			JSONUtils.getMorpherRegistry().registerMorpher(
					new DateMorpher(new String[] { "yyyy-MM-dd",
							"yyyy-MM-dd HH:mm:ss" }));
			XwKyzzb xwKyzzb = (XwKyzzb) JSONObject.toBean(json, XwKyzzb.class);
			xwKyzzbService.add(xwKyzzb);
	...
	public String Del() {
		try {
			...
			ids = "'" + ids.replace(",","','") + "'";
			String hql ="delete XwKyzzb where id in (" + ids + ")"; 
			xwKyzzbService.hqlExcute(hql);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
	...
	public String Del1(){
		try {
			String ids = ServletActionContext.getRequest().getParameter("ids");
			xwKyzzbService.deleteByIds(ids);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
...
//		filter = " (fyh='" + szdwh + "') ";
		...
		Page<XwKyzzb> pageList = xwKyzzbService.pageQuery(startNo,
	...
	public String toExcel(){
		...
		if(!excelParams.equals("")){
			...
			try {
				...
				List<?> tList = xwKyzzbService.Query(filters, order);
...
//			String order = null;
//			List<XwKyzzb> list =  (List<XwKyzzb>)this.xwKyzzbService.Query(filter,order);
//			
//			Date date = new Date();
//			
...
//	}
	public String changeFlowFlag() {
		try{
			xwKyzzbService.changeFlowFlag(xh,kyuuid,ztm,zt,shrgh,shr,shsj,shyj,shjgm,shjg);
			result.setSuccess(true);
		}catch(Exception e){
			result.setSuccess(false);
	...
	public String changeFlowFlagg() {
		try{
			xwKyzzbService.changeFlowFlagg(xh,kyuuid,ztm,zt,zsrgh,zsr,zssj,zsyj,shjgm,shjg);
			result.setSuccess(true);
		}catch(Exception e){
			result.setSuccess(false);
			result.setMsg("读取数据失败！");
	...
	public String GetKYUUID(){
		try{
			...
			setKyuuid(this.xwKyzzbService.getKyuuid(KYUUID));
			result.setSuccess(true);
		}catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
