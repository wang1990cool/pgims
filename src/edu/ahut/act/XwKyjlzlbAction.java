...
public class XwKyjlzlbAction extends ActionSupport{
...
//	private ViewXwKtbgsqService viewXwKtbgsqService;
	...
	public XwKyjlzlbAction() {
		xwKyjlzlbService = (XwKyjlzlbService) SpringContextUtil
				.getBean("XwKyjlzlbServiceImpl");
...
//	}
	...
	public String Add(){
		...
		try {
			XwKyjlzlb xwKyjlzlb = (XwKyjlzlb) JSONObject.toBean(json, XwKyjlzlb.class);
			xwKyjlzlbService.add(xwKyjlzlb);
	...
	public String Edit() {
		...
		try {
			XwKyjlzlb xwKyjlzlb = (XwKyjlzlb) JSONObject.toBean(json, XwKyjlzlb.class);
			xwKyjlzlbService.update(xwKyjlzlb);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
...
//			xwKyjlzlbService.hqlExcute(hql);
//			result.setSuccess(true);
//		} catch (Exception e) {
//			e.printStackTrace();
//			result.setSuccess(false);
...
//		filter = "xh = '" + xh + "'";
		...
		String filters = CommonUtil.MergeFilter(filter, searchParams);
		Page<XwKyjlzlb> pageList = xwKyjlzlbService.pageQuery(startNo, pageSize, filters, orders);
		setResult(new JsonResult(pageList.getResult(), pageList.getTotalCount()));
		result.setSuccess(true);
		return SUCCESS;
	}
	...
	public String toExcel(){
		...
		if(!excelParams.equals("")){
			...
			try {
				...
				List<?> tList = xwKyjlzlbService.Query(filters, order);
	...
	public String Del(){
		try {
			String ids = ServletActionContext.getRequest().getParameter("ids");
			xwKyjlzlbService.deleteByIds(ids);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
	...
	public String Del1(){
		try {
			String ids = ServletActionContext.getRequest().getParameter("ids");
			xwKyjlzlbService.deleteByIds1(ids);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
	...
	public String FileUploadJl() {
		System.out.println("FileUploadJl");
		return "pfileUploadJl";
	}
	
	public String FileDeleteJl() {
		return "pfileDeleteJl";
	}
	
	public String FileDownloadJl() {
		return "pfileDownloadJl";
	}
