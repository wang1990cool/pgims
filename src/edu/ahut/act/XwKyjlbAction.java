...
public class XwKyjlbAction extends ActionSupport{

	private static final long serialVersionUID = 4183786154194055246L;
	private String filter = null;
	private List<?> datas;
	private XwKyjlbService xwKyjlbService;
...
//	private String shjg;
	private Date zssj;
	public XwKyjlbAction() {
		xwKyjlbService = (XwKyjlbService) SpringContextUtil
				.getBean("XwKyjlbServiceImpl");
	}
	
	public XwKyjlbService getJxSkxxbService() {
		return xwKyjlbService;
	}

	public void setXwKyjlbService(XwKyjlbService xwKyjlbService) {
		this.xwKyjlbService = xwKyjlbService;
	}
...
//	}
		...
		try {
			JSONUtils.getMorpherRegistry().registerMorpher(
					new DateMorpher(new String[] { "yyyy-MM-dd",
							"yyyy-MM-dd HH:mm:ss" }));
			XwKyjlb xwKyjlb = (XwKyjlb) JSONObject.toBean(json, XwKyjlb.class);
			xwKyjlbService.add(xwKyjlb);
	...
	public String Del() {
		try {
			...
			ids = "'" + ids.replace(",","','") + "'";
			String hql ="delete XwKyjlb where id in (" + ids + ")"; 
			xwKyjlbService.hqlExcute(hql);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
	...
	public String Del1(){
		try {
			String ids = ServletActionContext.getRequest().getParameter("ids");
			xwKyjlbService.deleteByIds(ids);
			result.setSuccess(true);
		} catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
...
//		filter = " (fyh='" + szdwh + "') ";
		...
		Page<XwKyjlb> pageList = xwKyjlbService.pageQuery(startNo,
	...
	public String toExcel(){
		...
		if(!excelParams.equals("")){
			...
			try {
				...
				List<?> tList = xwKyjlbService.Query(filters, order);
...
//			String order = null;
//			List<XwKyjlb> list =  (List<XwKyjlb>)this.xwKyjlbService.Query(filter,order);
//			
//			Date date = new Date();
//			
...
//	}
	public String changeFlowFlag() {
		try{
			xwKyjlbService.changeFlowFlag(xh,kyuuid,ztm,zt,shrgh,shr,shsj,shyj,shjgm,shjg);
			result.setSuccess(true);
		}catch(Exception e){
			result.setSuccess(false);
	...
	public String changeFlowFlagg() {
		try{
			xwKyjlbService.changeFlowFlagg(xh,kyuuid,ztm,zt,zsrgh,zsr,zssj,zsyj,shjgm,shjg);
			result.setSuccess(true);
		}catch(Exception e){
			result.setSuccess(false);
			result.setMsg("读取数据失败！");
	...
	public String GetKYUUID(){
		try{
			...
			setKyuuid(this.xwKyjlbService.getKyuuid(KYUUID));
			result.setSuccess(true);
		}catch (Exception e) {
			e.printStackTrace();
			result.setSuccess(false);
